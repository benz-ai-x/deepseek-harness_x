...
provider 通过 `bindMemberOperations` 接收 grant。注册表只允许挂载已接受 handle 的代际获得权限。注册退役、handle 释放以及 Lead 的 `agent/disposed` 事件会中止其 signal，阻止后续查询返回数据。provider binder 抛错时，注册表撤销 grant 并隔离该代际，同时保留已经接受的 Team 事实。
