...
The catalog describes provider support, not tools installed in every historical native session. Create and resume results can attest `memberOperations` for their exact accepted handle. The registry owns this frozen subset beside live presence, scoped to the accepting provider generation. The roster exposes it without persisting it or granting authority. Presence-only updates preserve the proof; resume replaces it, and detachment removes it. An omitted proof remains unknown rather than inheriting the catalog or a retired generation's conclusion.
...
**Infer existing member tools from current provider metadata.** Rejected because immutable native sessions can retain an older tool set after a provider upgrade. A separate durable tool inventory would also claim recoverability that a provider without an installed-tool query cannot independently verify. Process-live acceptance facts preserve this uncertainty without changing native identity or Team formats.
...
The registry regression verifies per-handle isolation, detached operation arrays, preservation through presence, delivery and interrupt, proof removal on resume, same-id replacement, and disposal. Ultra's consumer regressions cover immutable historical Codex tools across cold recovery and a newly created thread's confirmed tools; authenticated native acceptance remains outside these deterministic checks.
