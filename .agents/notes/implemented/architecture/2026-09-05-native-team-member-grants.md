...
The provider receives the grant through `bindMemberOperations`. The registry admits only the generation attached to the accepted handle. Registration retirement, handle disposal, and the Lead's `agent/disposed` event abort its signal before further queries can return data. A throwing provider binder revokes its grant and quarantines that generation while retaining already accepted Team facts.
