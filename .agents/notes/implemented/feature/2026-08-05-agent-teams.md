...
Creation first appends and flushes a `team/member` provisioning snapshot in the Lead Session, then starts the reserved continuable child through the selected fresh or fork provider. Optional normalized per-child Agent options are retained as the requested route and passed unchanged to that continuation. Failure before initial inbox acceptance appends a failed snapshot. Success flushes the child's accepted inbox item, reads its descriptor-resolved provider/model/reasoning route, rejects any changed explicit request field, and then appends active with both routes. Recovery recognizes that initial message while it is still pending or after it enters user-message history. Names are reserved by the first provisioning record and never reused, including after failure. Disposal closes admission, aborts and awaits admitted creation and mailbox-dispatch transactions, then stops every live child recorded by the roster; a failed child remains cleanup-owned until its Activation exits, and cleanup rejection fails disposal.
