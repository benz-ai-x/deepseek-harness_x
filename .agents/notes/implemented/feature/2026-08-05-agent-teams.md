...
Creation first appends and flushes a `team/member` provisioning snapshot in the Lead Session, then starts the reserved continuable child through the selected fresh or fork provider. Optional normalized per-child Agent options are retained as the requested route and passed unchanged to that continuation. The launch caller owns cancellation through the initial inbox durability checkpoint. Before that acceptance, cancellation appends a failed snapshot and drains any child; afterward, cancellation ownership transfers to the Team lifecycle so caller disconnect cannot strand accepted work between descriptor correlation and the active roster commit. Success reads the child's descriptor-resolved provider/model/reasoning route, rejects any changed explicit request field, and then appends active with both routes. Recovery recognizes that initial message while it is still pending or after it enters user-message history. Names are reserved by the first provisioning record and never reused, including after failure. Disposal closes admission, aborts and awaits admitted creation and mailbox-dispatch transactions, then stops every live child recorded by the roster; a failed child remains cleanup-owned until its Activation exits, and cleanup rejection fails disposal.

A root recovery reconciles an unterminated provisioning record against the child's independently persisted Session. Matching direct parent, continuation provider, requested route, continuable descriptor, and a recorded initial user message prove successful admission and produce active with the descriptor route; absence, corruption, mismatched route/lineage, or a missing admitted message produces failed. An explicit child route is therefore independent of later Lead and deployment defaults, with no roster fallback to either. The creator re-reads the terminal phase under the same Lead-log serializer; if recovery marked failed while creation succeeded, it drains the child and reports a provisioning conflict instead of retaining an orphan. This avoids reconstructing an initial prompt that was never retained in the Team log and contains plugin-reload races.
...
`wait_agent` blocks on one roster, mailbox, task, or live-status edge registered after the call starts instead of encouraging model polling. It does not replay an earlier edge, so callers re-read authoritative state after wakeup or timeout. The browser watch uses the same post-commit activity owner but begins every reconnect generation with a full baseline before bounded invalidations. Lead-only interruption cancels the current turn with inbox preservation and does not alter mailbox or task ownership.
