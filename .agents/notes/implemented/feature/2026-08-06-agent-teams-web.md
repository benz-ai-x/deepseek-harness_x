...
Every task update sends the displayed revision. Create and edit forms expose current real task ids as native dependency checkboxes and exclude the edited task. Task text, scopes, and the complete dependency draft commit through one `edit` compare-and-set mutation; the Host validates references, authority, self-dependencies, and indirect cycles before appending the next revision. Each create or update owns an independent pending token, invalidates older refreshes before starting, and reloads the complete Team view after success. An edit conflict reloads the current authoritative task while retaining the old form and dependency draft with explicit unsaved feedback and no automatic retry; a reload failure remains visible. Overlapping refreshes publish only the latest request for the selected Session. Display mode, selected id, filter text, and viewport transform remain disposable component state rather than a durable task projection.
...
Team-service tests cover fixed-window pagination, filters, message and task-detail authorization, sanitized content, participant forgery, stale identity, cursor scope and corruption, persistence failure, no-activity reads, cold restart, atomic dependency edits, DAG validation, and rejected-write event counts. Per-file coverage fixes every message-reader path; generation plus a plain-Node built-artifact smoke verifies exported Remote descriptors. Client typechecking and browser component tests cover owner-declared child navigation, dynamic registration and disposal, the mounted namespace, Lead routing, shared list/graph selection, real edge direction and task facts, layout controls, keyboard and list alternatives, filtering, native dependency selection, conflicting drafts, task controls, stale async results, and bilingual status or error presentation. The keyless Agent Team profile snapshot exercises metadata and sanitized-detail reads through the real Host service, and the Web end-to-end test pins the single navigable panel over the real Remote composition.
