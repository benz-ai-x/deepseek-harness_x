...
The Team UI Fiber also owns the public Client-only `ctx.agentTeamPanelNavigation` service. An extension may address the exact Team Session, a registered child-view id, and an optional member id. The owner retains the request until the matching panel and child exist, then opens the same dialog, selects that child, forwards the member plus a monotonic revision through Slot owner props, and consumes the request once. This navigation hint neither reads Host data nor grants authority; the child still uses its own generated Remote and exact live-Lead checks.
...
Team-service tests cover fixed-window pagination, filters, detail authorization, sanitized content, participant forgery, stale identity, cursor scope and corruption, persistence failure, no-activity reads, and cold restart. Per-file coverage fixes every message-reader path; generation plus a plain-Node built-artifact smoke verifies exported Remote descriptors. Client typechecking and browser component tests cover owner-declared child navigation, dynamic registration and disposal, the mounted namespace, Lead routing, task controls, stale async results, and status or error presentation. Addressed-navigation cases additionally cover exact Team matching, one Team dialog, child selection, member/revision Slot props, single consumption, and removal of the navigation service with its owning Fiber. The keyless Agent Team profile snapshot exercises metadata and sanitized-detail reads through the real Host service, and the Web end-to-end test pins the single navigable panel over the real Remote composition.
