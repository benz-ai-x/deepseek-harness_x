...
const validPayloads: Readonly<Record<string, SessionFormatJsonValue>> = {
  ...
  'team/message/request-committed': {
    version: 1, teamId: 'team-1',
    receipt: { requestId: 'request-1', senderId: 'lead', inputFingerprint: 'a'.repeat(64), replyTo: 'prior-message',
      result: { requestId: 'request-1', messageId: 'message-1', status: 'accepted' } },
    message: { id: 'message-1', senderId: 'lead', senderName: 'lead', targetId: 'member-1', content: [textBlock] },
  },
  'team/native-operation/committed': {
    version: 4, kind: 'task', teamId: 'team-1',
    receipt: { id: 'b'.repeat(64), memberId: 'member-1', provider: 'native', nativeHandle: 'handle-1',
      source: { kind: 'tool', turnId: 'turn-1', callId: 'call-1' }, inputFingerprint: 'c'.repeat(64),
      request: { operation: 'tasks.update', taskId: 'task-1', expectedRevision: 1, action: 'edit',
        subject: 'subject', description: 'description', blockedBy: [], writeScopes: ['/work'], owner: 'worker' },
      result: { ok: true, operation: 'tasks.update',
        value: { task: { id: 'task-1', revision: 2, status: 'pending', ready: true, ownerName: 'worker' } } } },
    task: { id: 'task-1', revision: 2, subject: 'subject', description: 'description', status: 'pending',
      ownerId: 'member-1', blockedBy: [], writeScopes: ['/work'] },
  },
  'team/task': {
    version: 1, teamId: 'team-1',
    task: {
