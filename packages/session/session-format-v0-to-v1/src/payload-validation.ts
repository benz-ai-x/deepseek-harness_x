...
export function assertReleasedPayloadSemantics(event: SessionFormatEvent, version: number): void {
  ...
  switch (event.type) {
    ...
    case 'team/member':
      ...
      teamMemberValue(data['member'], `${label} member`, data['version'])
      return
    case 'team/message/delivered':
      teamSelector(data, label)
      nonEmptyString(data['messageId'], `${label} messageId`)
      nonEmptyString(data['targetId'], `${label} targetId`)
      if (data['nativeTurnId'] !== undefined) {
        literalValue(data['version'], [2], `${label} version`)
        teamOpaqueIdValue(data['nativeTurnId'], `${label} nativeTurnId`)
      }
      return
    ...
    case 'team/message/request-committed':
      ...
      teamMessageRequestValue(data['receipt'], `${label} receipt`)
      teamMessageValue(data['message'], `${label} message`, version, 2)
      return
    case 'team/native-operation/committed':
      teamNativeOperationValue(data, label, version)
      return
    case 'team/task':
      teamSelector(data, label)
...
function messageSourceValue(
  ...
  switch (source['kind']) {
    ...
    case 'team-message':
      ...
      if (source['senderName'] !== undefined) stringValue(source['senderName'], `${label} senderName`)
      return
    case 'goal':
...
function teamMemberValue(
  value: SessionFormatJsonValue | undefined, label: string, payloadVersion: SessionFormatJsonValue | undefined,
): void {
  const member = exactRecord(value, label, ['id', 'name', 'description', 'provider', 'context', 'phase'],
    payloadVersion === 2 ? ['error', 'requestedRoute', 'resolvedRoute', 'externalRuntime'] : ['error'])
  ...
  for (const key of ['requestedRoute', 'resolvedRoute'] as const) {
    if (member[key] !== undefined) {
      const route = exactRecord(member[key], `${label} ${key}`, [], ['provider', 'model', 'reasoningEffort'])
      for (const field of ['provider', 'model', 'reasoningEffort'] as const) {
        if (route[field] !== undefined) nonEmptyString(route[field], `${label} ${key} ${field}`)
      }
    }
  }
  if (member['externalRuntime'] !== undefined) {
    const runtime = teamExternalRuntimeValue(member['externalRuntime'], `${label} externalRuntime`)
    const requirements = releasedV0Record(runtime['requirements'], `${label} requirements`)
    if (member['requestedRoute'] !== undefined || member['resolvedRoute'] !== undefined
      || (member['phase'] === 'active' && runtime['nativeHandle'] === undefined)
      || (member['phase'] === 'provisioning' && runtime['nativeHandle'] !== undefined)
      || requirements['contextMode'] !== member['context']) {
      throw new SessionFormatError(`${label} external runtime conflicts with its route, phase, or context`)
    }
  }
}
...
function teamMessageValue(
  ...
  label: string,
  version: number,
  payloadVersion: SessionFormatJsonValue | undefined,
): void {
  const message = exactRecord(value, label, ['id', 'senderId', 'senderName', 'targetId', 'content',
    ...payloadVersion === 1 ? ['delivery'] : []])
  ...
  if (payloadVersion === 1) literalValue(message['delivery'], ['quiet', 'wakeup'], `${label} delivery`)
  contentBlocksValue(message['content'], `${label} content`, version)
}

// Frozen maintained-fork fields; importing the live experimental Team owner
// would let a later release silently change historical migration semantics.
function teamOpaqueIdValue(value: SessionFormatJsonValue | undefined, label: string): void {
  nonEmptyString(value, label)
  if (new TextEncoder().encode(value).byteLength > 200) {
    throw new SessionFormatError(`${label} exceeds 200 UTF-8 bytes`)
  }
}

function teamDigestValue(value: SessionFormatJsonValue | undefined, label: string): void {
  if (typeof value !== 'string' || !/^[0-9a-f]{64}$/u.test(value)) {
    throw new SessionFormatError(`${label} must be a lowercase SHA-256 digest`)
  }
}

function teamCapabilitiesValue(value: SessionFormatJsonValue | undefined, label: string, allowed: readonly string[]): void {
  const members = arrayValue(value, label, (member, memberLabel) => {
    literalValue(member, allowed, memberLabel)
  })
  const canonical = allowed.filter(member => members.includes(member))
  if (canonical.length !== members.length || canonical.some((member, index) => members[index] !== member)) {
    throw new SessionFormatError(`${label} must be unique and canonical`)
  }
}

function teamExternalRuntimeValue(value: SessionFormatJsonValue | undefined, label: string): JsonRecord {
  const runtime = exactRecord(value, label, ['kind', 'launchRequestId', 'requestFingerprint', 'requirements'],
    ['nativeHandle', 'initialTurnId'])
  literalValue(runtime['kind'], ['external-agent'], `${label} kind`)
  teamOpaqueIdValue(runtime['launchRequestId'], `${label} launchRequestId`)
  teamDigestValue(runtime['requestFingerprint'], `${label} requestFingerprint`)
  for (const key of ['nativeHandle', 'initialTurnId'] as const) {
    if (runtime[key] !== undefined) teamOpaqueIdValue(runtime[key], `${label} ${key}`)
  }
  const requirements = exactRecord(runtime['requirements'], `${label} requirements`,
    ['contextMode', 'profileCapabilities', 'runtimeCapabilities'])
  literalValue(requirements['contextMode'], ['fresh', 'fork'], `${label} contextMode`)
  teamCapabilitiesValue(requirements['profileCapabilities'], `${label} profileCapabilities`,
    ['persona', 'mission', 'context', 'memory', 'tool-policy', 'hooks'])
  teamCapabilitiesValue(requirements['runtimeCapabilities'], `${label} runtimeCapabilities`,
    ['full-collaboration', 'workspace-write', 'exact-call-approval', 'sandbox', 'evaluation', 'evidence', 'usage'])
  const profile = requirements['profileCapabilities'] as SessionFormatJsonValue[]
  if (!profile.includes('persona') || !profile.includes('mission')) {
    throw new SessionFormatError(`${label} profileCapabilities requires persona and mission`)
  }
  return runtime
}

function teamMessageRequestValue(value: SessionFormatJsonValue | undefined, label: string): void {
  const receipt = exactRecord(value, label, ['requestId', 'senderId', 'inputFingerprint', 'result'], ['replyTo'])
  teamOpaqueIdValue(receipt['requestId'], `${label} requestId`)
  nonEmptyString(receipt['senderId'], `${label} senderId`)
  teamDigestValue(receipt['inputFingerprint'], `${label} inputFingerprint`)
  if (receipt['replyTo'] !== undefined) nonEmptyString(receipt['replyTo'], `${label} replyTo`)
  const result = exactRecord(receipt['result'], `${label} result`, ['requestId', 'messageId', 'status'])
  teamOpaqueIdValue(result['requestId'], `${label} result requestId`)
  nonEmptyString(result['messageId'], `${label} result messageId`)
  literalValue(result['status'], ['accepted'], `${label} result status`)
}

function teamNativeOperationValue(data: JsonRecord, label: string, sessionVersion: number): void {
  literalValue(data['version'], [3, 4], `${label} version`)
  nonEmptyString(data['teamId'], `${label} teamId`)
  if (data['version'] === 4) literalValue(data['kind'], ['message', 'task'], `${label} kind`)
  const task = data['version'] === 4 && data['kind'] === 'task'
  assertReleasedV0Keys(data, ['version', 'teamId', 'receipt', task ? 'task' : 'message',
    ...data['version'] === 4 ? ['kind'] : []], [], label)
  const receipt = exactRecord(data['receipt'], `${label} receipt`,
    ['id', 'memberId', 'provider', 'nativeHandle', 'source', 'inputFingerprint', 'result', ...task ? ['request'] : []])
  teamDigestValue(receipt['id'], `${label} receipt id`)
  teamDigestValue(receipt['inputFingerprint'], `${label} receipt inputFingerprint`)
  nonEmptyString(receipt['memberId'], `${label} memberId`)
  nonEmptyString(receipt['provider'], `${label} provider`)
  teamOpaqueIdValue(receipt['nativeHandle'], `${label} nativeHandle`)
  const source = releasedV0Record(receipt['source'], `${label} source`)
  literalValue(source['kind'], ['tool', 'settlement'], `${label} source kind`)
  assertReleasedV0Keys(source, ['kind', 'turnId', ...source['kind'] === 'tool' ? ['callId'] : []], [], `${label} source`)
  teamOpaqueIdValue(source['turnId'], `${label} turnId`)
  if (source['kind'] === 'tool') teamOpaqueIdValue(source['callId'], `${label} callId`)
  const result = exactRecord(receipt['result'], `${label} result`, ['ok', 'operation', 'value'])
  literalValue(result['ok'], [true], `${label} result ok`)
  if (task) {
    teamTaskValue(data['task'], `${label} task`)
    teamNativeTaskRequestValue(receipt['request'], `${label} request`)
    literalValue(result['operation'], ['tasks.update'], `${label} result operation`)
    const resultValue = exactRecord(result['value'], `${label} result value`, ['task'])
    const accepted = exactRecord(resultValue['task'], `${label} accepted task`, ['id', 'revision', 'status', 'ready'], ['ownerName'])
    nonEmptyString(accepted['id'], `${label} accepted id`)
    positiveIntegerValue(accepted['revision'], `${label} accepted revision`)
    literalValue(accepted['status'], ['pending', 'in_progress', 'completed', 'deleted'], `${label} accepted status`)
    booleanValue(accepted['ready'], `${label} accepted ready`)
    if (accepted['ownerName'] !== undefined) stringValue(accepted['ownerName'], `${label} accepted ownerName`)
  } else {
    teamMessageValue(data['message'], `${label} message`, sessionVersion, 2)
    literalValue(result['operation'], ['messages.send', 'turns.settle'], `${label} result operation`)
    const accepted = exactRecord(result['value'], `${label} result value`,
      ['messageId', 'status', ...result['operation'] === 'turns.settle' ? ['outcome'] : []])
    nonEmptyString(accepted['messageId'], `${label} result messageId`)
    literalValue(accepted['status'], ['queued'], `${label} result status`)
    if (result['operation'] === 'turns.settle') {
      literalValue(accepted['outcome'], ['completed', 'failed', 'interrupted'], `${label} result outcome`)
    }
  }
}

function teamNativeTaskRequestValue(value: SessionFormatJsonValue | undefined, label: string): void {
  const request = exactRecord(value, label, ['operation', 'taskId', 'expectedRevision', 'action'],
    ['subject', 'description', 'blockedBy', 'writeScopes', 'owner'])
  literalValue(request['operation'], ['tasks.update'], `${label} operation`)
  teamNativeTaskIdValue(request['taskId'], `${label} taskId`)
  positiveIntegerValue(request['expectedRevision'], `${label} expectedRevision`)
  literalValue(request['action'], ['claim', 'release', 'edit', 'set_dependencies', 'complete', 'reopen', 'reassign', 'delete'], `${label} action`)
  for (const key of ['subject', 'description', 'owner'] as const) {
    if (request[key] !== undefined) stringValue(request[key], `${label} ${key}`)
  }
  if (request['blockedBy'] !== undefined) arrayValue(request['blockedBy'], `${label} blockedBy`, teamNativeTaskIdValue)
  if (request['writeScopes'] !== undefined) arrayValue(request['writeScopes'], `${label} writeScopes`, stringValue)
}

function teamNativeTaskIdValue(value: SessionFormatJsonValue | undefined, label: string): void {
  nonEmptyString(value, label)
  if (value.length > 128) throw new SessionFormatError(`${label} exceeds 128 characters`)
}
