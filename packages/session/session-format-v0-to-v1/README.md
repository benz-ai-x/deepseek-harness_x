...
The frozen inventory also owns the maintained Team payload 2 fields, native operation payloads 3/4, and message request payload 1. It preserves routes, external identities, native delivery turns, requests and receipts under explicit nested validation; importing the live experimental Team schema would make historical decoding change with that service. The [maintained integration decision](../../../.agents/notes/implemented/architecture/2026-09-08-maintained-team-on-session-v2.md) distinguishes this support from complete cross-domain migration.
...
- **Closed build-owned inventory** — unknown external-plugin events refuse migration in this alpha policy.
