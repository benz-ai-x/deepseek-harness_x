...
冻结清单还负责维护版 Team payload 2 字段、native operation payload 3/4 和 message request payload 1。它通过显式嵌套校验保留路由、外部身份、原生投递 turn、请求和回执；导入实时实验性 Team schema 会使历史解码随该服务改变。[维护版集成决策](../../../.agents/notes/implemented/architecture/2026-09-08-maintained-team-on-session-v2.zh.md)区分本支持与完整跨业务域迁移。
...
- **封闭的构建所有者清单**——按照当前 Alpha 策略，未知的外部插件事件会使迁移失败。
