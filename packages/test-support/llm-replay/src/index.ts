...
function parseSessionFixture(text: string): ParsedSessionFixture {
  ...
  const nativeReceiptTokens = new Map<string, string>()
  ...
  for (const source of parsed.slice(1)) {
    ...
    const token = projectedNativeReceipt(source.value)?.['id']
    const materialized = projectedNativeReceipt(record)?.['id']
    if (typeof token === 'string' && typeof materialized === 'string' && token !== materialized) {
      nativeReceiptTokens.set(materialized, token)
    }
  ...
  try {
    ...
    const restored = { ...current, events: current.events.map((event) => {
      const id = projectedNativeReceipt(event)?.['id']
      const token = typeof id === 'string' ? nativeReceiptTokens.get(id) : undefined
      return token === undefined ? event : replaceProjectedNativeReceiptId(event, token) as unknown as typeof event
    }) }
    return parsedSessionFixture(restored, headerLine.value)
  } catch (error: unknown) {
...
function restoreProjectedRequestHeader(
  ...
  const token = projectedNativeReceipt(source)?.['id']
  if (typeof token === 'string' && /^\{\{id:[1-9]\d*\}\}$/u.test(token)) {
    return replaceProjectedNativeReceiptId(target, token)
  }
...
function normalizeProjectedRow(source: Readonly<Record<string, unknown>>): Record<string, unknown> {
  const token = projectedNativeReceipt(source)?.['id']
  const record = typeof token === 'string' && /^\{\{id:[1-9]\d*\}\}$/u.test(token)
    ? replaceProjectedNativeReceiptId(source, createHash('sha256').update(token).digest('hex'))
    : { ...source }
...
/** Locate only the explicitly tokenized native receipt field, never intentional content. */
function projectedNativeReceipt(row: Readonly<Record<string, unknown>>): Readonly<Record<string, unknown>> | undefined {
  if (row['type'] !== 'team/native-operation/committed') return undefined
  const data = row['data']
  if (data === null || typeof data !== 'object' || Array.isArray(data)) return undefined
  const receipt = (data as Record<string, unknown>)['receipt']
  return receipt === null || typeof receipt !== 'object' || Array.isArray(receipt)
    ? undefined
    : receipt as Record<string, unknown>
}

/** Copy one fixture-only identity replacement without mutating the borrowed row. */
function replaceProjectedNativeReceiptId(row: Readonly<Record<string, unknown>>, id: string): Record<string, unknown> {
  return { ...row, data: { ...row['data'] as Record<string, unknown>, receipt: {
    ...projectedNativeReceipt(row), id,
  } } }
}
