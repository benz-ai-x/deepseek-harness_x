...
| [`agent-team-claude-code`](agent-team-claude-code/README.md) | Persistent Claude Agent SDK Session backend for external teammates | registers a provider on `ctx.agentTeams` |
| [`agent-team-codex`](agent-team-codex/README.md) | Persistent Codex App Server backend for external teammates | registers a provider on `ctx.agentTeams` |
