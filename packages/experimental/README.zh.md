...
实验组包含不属于任何正式发布的原型能力：它们运行在真实 harness 上，但约定可能变更，也不提供支持承诺。本组包含 Agent Teams 及其持久 Codex 与 Claude Code 后端、跨 realm Inspector、代码执行 seam 的 CPython 子进程后端，以及预览部署使用的浏览器 worker 运行时与镜像打包器。用这些包来尝试未发布的能力；它们没有稳定性承诺，已发布产品不得依赖它们。
...
| [`agent-team-claude-code`](agent-team-claude-code/README.zh.md) | external teammate 的持久 Claude Agent SDK Session 后端 | 在 `ctx.agentTeams` 上注册 provider |
| [`agent-team-codex`](agent-team-codex/README.zh.md) | external teammate 的持久 Codex App Server 后端 | 在 `ctx.agentTeams` 上注册 provider |
