...
The exact live Lead may call `ctx.agentTeams.readTeammateRuntimeEvidence()` for one active external teammate. Agent Teams supplies the roster-owned native handle and returns only a bounded normalized page of turn/tool identities, outcomes, timestamps, and provider-reported usage. Prompts, replies, tool arguments/results, files, environment values, credentials, and raw provider payloads never cross the service boundary.
