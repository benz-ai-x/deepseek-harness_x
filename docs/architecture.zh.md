...
[实验性 Agent Teams](subsystems/agent-team.zh.md) 是 `ctx.agentTeams` 上的私有显式启用协作 seam。其持久 roster、任务板与 mailbox 会把每个 teammate 路由到可继续 DSH child，或带精确持久 handle、由 Fiber scoped 注册的外部 runtime provider。
