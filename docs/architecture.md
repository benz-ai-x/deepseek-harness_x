...
[Experimental Agent Teams](subsystems/agent-team.md) is a private opt-in coordination seam on `ctx.agentTeams`. Its durable roster, task board, and mailbox route each teammate through either a continuable DSH child or a Fiber-scoped external runtime provider with an exact durable handle.
