...
describe('web e2e: Agent Teams panel', () => {
  ...
  it('renders one authoritative task list and dependency graph through the single navigable Team panel', async () => {
    ...
    await action.getByPlaceholder('Task subject', { exact: true }).fill('Visible prerequisite')
    await action.getByPlaceholder('Task description', { exact: true }).fill('Visible prerequisite for navigation')
    await action.getByRole('button', { name: 'Save', exact: true }).click()
    await graph.getByRole('button', { name: 'task-3 · Visible prerequisite', exact: true }).waitFor()
    await action.getByRole('button', { name: 'New task', exact: true }).click()
    await action.getByPlaceholder('Task subject', { exact: true }).fill('Visible dependent')
    await action.getByPlaceholder('Task description', { exact: true }).fill('Depends on hidden and visible prerequisites')
    await action.getByRole('checkbox', { name: 'task-1 · Prepare inputs', exact: true }).check()
    await action.getByRole('checkbox', { name: 'task-3 · Visible prerequisite', exact: true }).check()
    await action.getByRole('button', { name: 'Save', exact: true }).click()
    const dependent = graph.getByRole('button', { name: 'task-4 · Visible dependent', exact: true })
    await dependent.waitFor()
    await action.getByRole('searchbox', { name: 'Filter tasks', exact: true }).fill('Visible')
    expect(await graph.getByRole('button', { name: 'task-1 · Prepare inputs', exact: true }).count()).toBe(0)
    await dependent.focus()
    await dependent.press('ArrowLeft')
    await action.getByRole('region', { name: 'Task details', exact: true })
      .getByText('task-3 · Visible prerequisite', { exact: true }).waitFor()
    expect(await graph.getByRole('button', { name: 'task-3 · Visible prerequisite', exact: true })
